...
export function applyFontPreview(
  ...
  const style = `
		...
		`;
  if (!previewFontsStylesheet) generatePreview();
...
function getActiveFontStylesheet(selectorSuffix: string): HTMLStyleElement {
  ...
  let activeFontStylesheet = document.getElementById(
    stylesheetId
  ) as HTMLStyleElement;
...
export function applyActiveFont(
  ...
  const style = `
    ...
    previousFontFamily ? `, "${previousFontFamily}"` : ""
  };
		}
	`;
